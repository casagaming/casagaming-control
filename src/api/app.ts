    ...
    const orders = ordersResult.rows.map(r => {
      ...
      o.order_items = items
        ...
        .map(i => ({
          ...
          quantity: i.quantity,
          price: i.price,
          product: i.prod_name_en ? { name_en: i.prod_name_en, image_url: toArr(i.prod_image_url) } : null,
          variant: i.var_id ? { id: i.var_id, name_en: i.var_name_en, image_url: i.var_image_url } : null
        }));
      return o;
    });
    res.json(orders);
