...
async function test() {
  try {
    const products = await db.execute("PRAGMA table_info(products)");
    console.log('Products Schema:', products.rows);
    const categories = await db.execute("PRAGMA table_info(categories)");
    console.log('Categories Schema:', categories.rows);
  } catch (e) {
